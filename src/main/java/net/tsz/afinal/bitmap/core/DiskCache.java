...
public class DiskCache implements Closeable {
	...
	public DiskCache(String path, int maxEntries, int maxBytes, boolean reset,
		...
		final File fileParent = dir.getParentFile();
	...
	public void delete() {
		...
		try {
			...
			mIndexFile = new RandomAccessFile(mPath + ".idx", "rw");
			mDataFile0 = new RandomAccessFile(mPath + ".0", "rw");
			mDataFile1 = new RandomAccessFile(mPath + ".1", "rw");

			resetCache(mMaxEntries, mMaxBytes);
	...
	int getActiveCount() {
		...
		if (count == mActiveEntries) {
			...
			// Log.e(TAG, "wrong active count: " + mActiveEntries + " vs " +
			// count);
	...
	public long getCacheSize() {
		...
		try {
			if (mIndexFile != null) {
				size += mIndexFile.length();
			}
			if (mDataFile0 != null) {
				size += mDataFile0.length();
			}
			if (mDataFile1 != null) {
				size += mDataFile1.length();
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
