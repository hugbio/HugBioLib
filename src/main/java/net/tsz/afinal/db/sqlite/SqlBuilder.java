...
public class SqlBuilder {
    ...
    /***
     * 获取 Alter语句。支持ADD(插入列)
     */
    public static String getAlterSql(String alterType, String tableName, String cloumnName, Class<?> dataType) {
        StringBuffer strSQL = new StringBuffer();
        strSQL.append("ALTER TABLE ");
        strSQL.append(tableName);
        strSQL.append(" ");
        strSQL.append(alterType);
        strSQL.append(" ");
        strSQL.append(cloumnName);
        if (dataType != null) {
     ...
     * 数据库列的数据类型
     */
    public static String getDataTypeSql(Class<?> dataType) {
        if (dataType == int.class || dataType == Integer.class
                || dataType == long.class || dataType == Long.class) {
            return "INTEGER";
        } else if (dataType == float.class || dataType == Float.class
     ...
     *获取复制表数据的Sql语句
     */
    public static String getCopyTableDataSql(String oldTableName, String newTableName, ArrayList<String> columns){
        StringBuffer strSQL = new StringBuffer();
        strSQL.append("INSERT INTO ");
        strSQL.append(newTableName);
        String cs = "";
    ...
    public static String getCreatTableSQL(Class<?> clazz) {
        TableInfo table = TableInfo.get(clazz);
        return getCreatTableSQL(table);
    }

    public static String getCreatTableSQL(TableInfo table) {
        Id id = table.getId();
        StringBuffer strSQL = new StringBuffer();
